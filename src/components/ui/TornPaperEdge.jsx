...
// Option 8: Torn Paper Edge Transition
const TornPaperEdge = ({ className = '', fill = "var(--fallback-b1,oklch(var(--b1)))", top = false }) => {
  return (
    <div className={`w-full overflow-hidden leading-none pointer-events-none absolute left-0 right-0 ${top ? 'top-0 rotate-180' : 'bottom-0'} ${className}`} style={{ height: '40px', zIndex: 10 }}>
      <svg 
        viewBox="0 0 1200 120" 
        preserveAspectRatio="none" 
        className="relative block w-full h-[40px]"
        style={{ filter: 'drop-shadow(0px -5px 2px rgba(0,0,0,0.05))' }}
      >
        {/* Jagged path simulating torn paper */}
        <path 
